...
const NavBarDesktop = () => {
    ...
    // State to hold the retrieved object
    ...
    return (
        <div className="nav-container h-100">
            <div className="shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] absolute left-0 top-0 z-1 flex h-100 w-96 flex-col overflow-y-hidden bg-white duration-300 ease-linear dark:bg-boxdark md:static md:translate-x-0 -translate-x-full">
                <div className="flex items-center justify-between gap-2 px-6 py-4 md:py-6.5">
                    <a className="">
                        <img className="w-36 mt-4" src="https://www.dol-sensors.com/wp-content/uploads/2021/06/dol-sensors-long_blue.png" alt="dol-logo"></img>
                    </a>
                </div>
                <Nav vertical className="h-full">
                    {isAuthenticated && (
                        <NavItem>
                            <NavLink
                                className="group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-dol-blue duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 false"
                                tag={RouterNavLink}
                                to="/dashboard"
                                end
                            ...
                            ><svg className="w-6 h-6 text-gray-800 dark:text-dol-blue" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    ...
                    {isAuthenticated && details && (
                        <div>
                            <h3 className="group opacity-80 relative flex items-center gap-2.5 rounded-sm py-3 px-4 font-medium text-dol-blue duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 false">
                                DEVICES
                            </h3>
                            {details.map((device, key) => (
                                <NavItem key={key}>
                                    <NavLink
                                        className="group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-dol-blue duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 false"
                                        tag={RouterNavLink}
                                    ...
                                    {device.sensors !== 0 && device.deviceType !== "IDOL65" && (
                                        device.sensors.map((sensor, key) => (
                                            <NavLink
                                                className="group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-dol-blue duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 false"
                                                tag={RouterNavLink}
                                                key={key}
                                                state={{ item: sensor, mac: device.mac, deviceStatus: deviceStatus }}
                                                to={`/devices/${device.mac}/sensors/${sensor.devEui}`}
                                            >
                                                <div className="mx-4">
                                                    {sensor.name}
                                                </div>
                    ...
                    {isAuthenticated && (
                        <NavItem className="mt-auto">
                            <div className="group relative flex items-center gap-2.5 rounded-sm py-4 px-4 font-medium text-dol-blue duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 false">
                                <Link to={"/profile"}>
                                    <img
                                        src={user.picture}
                                        alt="Profile"
                                ...
                                <RouterNavLink onClick={() => logoutWithRedirect()}>
                                    Log out
                                </RouterNavLink>
                            </div>
                        </NavItem>
